...
const
  astap_version='2026.07.15';  //  astap_version := {$I %DATE%} + ' ' + {$I %TIME%});
type
...
function save_fits_compressed(img: Timage_array; memo: tstrings; headX: theader;
                              filen2: ansistring; override2: boolean): boolean;
const
  card_size = 80;
var
  TheFile   : tfilestream;
  colours5, width5, height5 : integer;
  y, x, i, outlen, clen, pcount, hcount, acc : integer;
  row16     : PWord;
  cbuf      : PByte;
  errmsg    : string;
...
begin
  ...
  begin
    memo2_message('Rice compression supports only monochrome 16-bit images.');
    exit;
  end;
  ...
  try
    ...
    getmem(row16, width5*2);
    clen := width5*2 + width5 div 16 + 64;   { safe upper bound, matches unit_ricecomp }
    getmem(cbuf, clen);
    pcount := 0;
    ok := true;
    try
      ...
      begin
        for x := 0 to width5-1 do
          { physical value -> BZERO/BSCALE-adjusted signed 16-bit, exactly as the
            normal 16-bit writer does: value - 32768, kept in a word }
          row16[x] := word(max(0, min(65535, round(img[0, y, x]))) - 32768);

        if not rice_encode(row16, 2, width5, 32, cbuf, clen, outlen, errmsg) then
        begin
          memo2_message('Rice compression error: ' + errmsg);
          ok := false;
          break;
        end;
        getmem(tile_data[y], outlen);
        move(cbuf^, tile_data[y]^, outlen);
        tile_len[y] := outlen;
        inc(pcount, outlen);
      end;
    finally
    ...
    try { Do some lengthy operation }
       ...
       begin
         ...
         begin
           if headx.bitpix=16 then
           begin
             if save_fits_compressed(img_temp,memox,headx,ChangeFileExt(Filename1,'fits.fz'),false {overwrite})=false then
               err:=true;;
           end;
         end
         else
...
//  else
  ...
  begin
    ...
    if idx=3 then
    begin
      save_fits_compressed(img_loaded,mainform1.memo1.lines,head,savedialog1.filename,false);
    end
    else
    if idx=4 then
    begin
