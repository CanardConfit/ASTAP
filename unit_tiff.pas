...
{ -------------------------------------------------------------------------
  Generic TIFF Read Function
  ------------------------------------------------------------------------- }
procedure read_tiffOLD(filen: string; var img: Timg_array;  out description: string; out bitspersample: word; out measured_max: double; out theresult: boolean);
var
  fs             : TFileStream;
  tiffbuffer     : array of Byte;
  ...
  var
    p16      : PWord;
    s, j, b  : Integer;
    rowBytes  : Integer;       // row_size in bytes
    numComps  : Integer;       // samples * (bits/8)
    temp      : array of Byte;
    srcPtr   : PByte;
  begin
    ...
    rowBytes := row_size;
    numComps := samplesperpixel;   // components per pixel for delta step

    // ── Step 1: undo horizontal differencing ──────────────────────────────────
    // For predictor 2 the delta stride equals samplesperpixel (in bytes for 8-
    // bit, in words for 16-bit).  For predictor 3 the differencing was done on
    // the *reordered* byte stream, so the stride is always 1 byte (the reordered
    // stream is treated as bytes), but we must work on the full rowBytes width.

    if predictor = 2 then
    begin
      if bitspersample = 8 then
      begin
        for j := samplesperpixel to rowBytes - 1 do
          tiffbuffer[j] := Byte(tiffbuffer[j] + tiffbuffer[j - samplesperpixel]);
      end
      else if bitspersample = 16 then
      begin
        // Undo word-level delta; use safe byte-by-byte to avoid alignment issues
        for j := samplesperpixel to (rowBytes div 2) - 1 do
        begin
          // Read little-endian word, add previous, write back
          p16 := PWord(@tiffbuffer[j * 2]);
          p16^ := Word(p16^ + PWord(@tiffbuffer[(j - samplesperpixel) * 2])^);
        end;
      end;
      // 32-bit integer predictor 2 would go here if needed
    end

    else if predictor = 3 then  // floating-point predictor
    begin
      // The encoder turned each float's bytes into 4 separate planes
      // (plane 0 = all byte-0 of every sample, plane 1 = all byte-1, …)
      // and then delta-coded each plane independently.
      // We must:
      //   (a) undo the per-plane byte delta
      //   (b) reassemble the interleaved IEEE 754 floats

      // (a) undo differencing — stride is samplesperpixel bytes across the plane
      //     Each plane has (rowBytes div 4) bytes for 32-bit floats
      //     But the differencing was applied with stride = samplesperpixel
      //     within the whole reordered row, so stride = samplesperpixel still.
      for j := samplesperpixel to rowBytes - 1 do
        tiffbuffer[j] := Byte(tiffbuffer[j] + tiffbuffer[j - samplesperpixel]);

      // (b) undo byte-plane reordering
      //     The reordered layout is:
      //       [ byte3 of all floats | byte2 of all floats |
      //         byte1 of all floats | byte0 of all floats ]
      //     where "all floats" means width * samplesperpixel floats.
      //     We need to reassemble each float from its scattered bytes.
      SetLength(temp, rowBytes);
      Move(tiffbuffer[0], temp[0], rowBytes);

      srcPtr := @temp[0];
      // numFloats = total number of 32-bit samples in the row
      // The 4 planes each occupy rowBytes div 4 bytes
      for j := 0 to (rowBytes div 4) - 1 do
      begin
        // TIFF FP predictor stores planes MSB-first: plane[3], plane[2],
        // plane[1], plane[0] maps to float bytes [3],[2],[1],[0] (big-endian
        // within the plane layout), but the actual IEEE float on a little-endian
        // machine needs byte[0]=LSB.  Reassemble accordingly:
        tiffbuffer[j * 4 + 3] := srcPtr[j];                         // MSB plane
        tiffbuffer[j * 4 + 2] := srcPtr[j +     rowBytes div 4];
        tiffbuffer[j * 4 + 1] := srcPtr[j + 2 * (rowBytes div 4)];
        tiffbuffer[j * 4 + 0] := srcPtr[j + 3 * (rowBytes div 4)];  // LSB plane
      end;
    end;
  end;
  ...
  var
     m,row : integer;
  begin
    row:=height-row_inv-1;//reverse in FITS, count from bottom
    if samplesperpixel = 1 then
    begin
      if bitspersample = 8 then
        for m := 0 to width - 1 do
          img[0, row, m] := tiffbuffer[m] * 257   //65535:=255*257
      else if bitspersample = 16 then
        for m := 0 to width - 1 do
          img[0, row, m] := PWord(@tiffbuffer[m*2])^
      else if is_float then
        for m := 0 to width - 1 do
        begin
          pixel_value:=PSingle(@tiffbuffer[m*4])^*65535;
          measured_max:=max(pixel_value,measured_max);
          img[0, row, m] :=pixel_value;
        end;
    end
    else
    begin
      ...
      begin
        for m := 0 to width - 1 do
        begin
          img[0, row, m] := tiffbuffer[m*3]   * 257;
          img[1, row, m] := tiffbuffer[m*3+1] * 257;
          img[2, row, m] := tiffbuffer[m*3+2] * 257;
        end;
      end
      else if bitspersample = 16 then
      begin
        for m := 0 to width - 1 do
        begin
          img[0, row, m] := PWord(@tiffbuffer[m*6])^;
          img[1, row, m] := PWord(@tiffbuffer[m*6+2])^;
          img[2, row, m] := PWord(@tiffbuffer[m*6+4])^;
        end;
      end
      else if is_float then
      begin
        for m := 0 to width - 1 do
        begin
          pixel_value:=PSingle(@tiffbuffer[m*12])^*65535;
          measured_max:=max(pixel_value,measured_max);
          img[0, row, m] := pixel_value;
          img[1, row, m] := PSingle(@tiffbuffer[m*12+4])^ * 65535;
          img[2, row, m] := PSingle(@tiffbuffer[m*12+8])^ * 65535;
        end;
      end;
    end;
  end;
...
begin
  ...
  try
    ...
    if (header[0] <> $49) or (header[1] <> $49) then
    begin
      description:='Unsupported TIFF byte order (big-endian not supported).';
      //MessageDlg(description, mtError, [mbOK], 0);
      Exit;
    end;
    ...
    begin
      ...
      if rowsperstrip < rows_in_strip then
        rows_in_strip := rowsperstrip;
      ...
      begin
        ...
        try
          ...
          lzwLen := 0;
          try
            DecompressLZW(mem_str.Memory, strip_bytecounts[i],
              lzwOut, lzwLen);
    ...
    //if measured_max is too large scale to 0..65535
    if measured_max>65535*1.5 then
    begin
      scalefactor:=65535/measured_max;  {rescale to 0..65535 float}
    ...
    if measured_max=0  then //data not in float format
        measured_max:=65535;
