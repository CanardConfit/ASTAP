...
{taken from HNSKY unit. Routines (c) by Springer-Nature unless indicated otherwise}
...
function strtofloat(st: string) : double;
var
  error2 : integer;
begin
...
procedure convert_MPCORB_line(txt : string; var desn: string; var H,G,yy,mm,dd,M,Peri,Node,a_incl,a_ecc,a_a: double); {by han}
var
...
function deltaT_calc(jd: double) : double; {in seconds}
var
   year   : integer;
   y,u,t  : double;
begin
...
procedure plot_mpcorb;{read MPCORB.dat}{by han}
const
   ...
   {5. Conversion (RA,DEC) -> (x,y)}
    ...
    begin
      {annotate}
       ...
       len:=20;
       mainwindow.image1.Canvas.textout(x2+len,y2,desn+'('+floattostrF(mag,ffgeneral,3,1)+')');
...
begin
  ...
  if cd1_1=0 then exit;end;{should not happen  menu disabled}
  ...
  mainwindow.image1.canvas.pen.color:=clFuchsia;{color circel}
  mainwindow.image1.Canvas.font.color:=clFuchsia;
  mainwindow.image1.Canvas.font.size:=12;

//  DATE_OBS:= '2020-02-01T00:00:00';
  sitelong:=-7;
  sitelat:=53;

  date_to_jd(date_avg);{convert date-AVG to jd}
  if jd=0 then {try with date_obs}
  begin
  ...
  assignfile(txtf,'mpcorb.dat');
  try
    Reset(txtf);
    while ((not EOF(txtf)) and (count<10000)) do   {loop}
    begin
      ReadLn(txtf, s);
      convert_MPCORB_line(s, {var} desn, a_h,a_g,yy,mm,dd,a_anm,Peri,Node,a_incl,a_ecc,a_a);
